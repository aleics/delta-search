...
#[grammar_inline = r#"
    ...
    ORDER_BY = { "ORDER" ~ "BY" ~ name ~ (SPACE_SEPARATOR* ~ (ASC | DESC))? }
    LIMIT    = { "LIMIT" ~ number }
...
mod tests {
    ...
    fn creates_complex_filter() {
        // given
        let input = "FROM person WHERE (person.name != \"Michael Jordan\") && (score > 1 || active = true && (person.name.simple = \"Roger\" || score <= 5))";
